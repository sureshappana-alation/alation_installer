...
echo "Waiting for Alation fat container pod "$pod " to become ready"
...
kubectl exec $pod -- bash -c "$aa_postgres_password_master_cmd"
kubectl exec $pod -- bash -c "$aa_ocf_alation_conf_master_cmd"
